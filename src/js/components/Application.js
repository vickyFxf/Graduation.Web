...
export default class Application extends React.Component {
                    ...
                    <div className="right">
                        <ul>
                            <li className='currentInfo'>欢迎您
                                <span>{this.state.id}1406010039</span>
                                <span>{this.state.userName}范秀芳</span>|
                                <span onclik={this.logout.bind(this)} className="logout">退出</span>
                            </li>
                        </ul>
                    </div>
                </Header>
    ...
    logout() {
        //先判定是否是第一次登录:初始登录密码为123456加密
        let data = {};
        data.id = this.state.id;
        GetUserInfo(data).then(res => {
            let mdPwd = hex_md5('123456');
            if (res.password == mdPwd) {
                const path = '/changePwd';
                browserHistory.push(path)
                // <Modal>
                //     请先修改密码
                // </Modal>
            } else {
                window.location.href = 'localhost:4000/login.html';
            }
        })
    }
}